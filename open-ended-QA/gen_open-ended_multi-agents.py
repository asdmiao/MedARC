...
def construct_message(agents, question, idx):
    if len(agents) == 0:
        return {"role": "user", "content": "Can you double check that your answer is correct. Please reiterate your answer, the answer should consist of one to two complete sentences."}
...
if __name__ == "__main__":
    ...
    for index, data in enumerate(questions):
        ...
        for round in range(rounds):
            for i, agent_context in enumerate(agent_contexts):
                ...
                # 定义API调用
                if i == 0:
                    api_call = lambda: client2.chat.completions.create(
                        model="glm-4-air",
    ...
    file_path = os.path.join(output_path, "open-ended_multi-agents_{}_{}.json".format(agents, rounds))
